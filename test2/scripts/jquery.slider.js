 ...
 * version: 2.0.0
 * author: creeper yang
 * description: a jQuery plugin, image slider.
*/
;(function($){
	$.fn.zImageSlider = function(options){
		...
		$content.on("touchstart", function(ev) {
			...
			moveStart(ev, touch[0].pageX);
		}).on("touchmove", function(ev){
			var touch = ev.touches || ev.originalEvent.touches;
			move(ev, touch[0].pageX);
		});
		...
		function displayItem(index){
			...
			$indicator.find(".image-indicator-dot").removeClass("active");
			$indicator.find(".image-indicator-dot:nth-child(" + (index + 1) + ")").addClass("active");
		};
	...
	function initIndicator($slider, $indicator, count, initIndex) {
		if(count > 1) {
			...
			for(var i=0; i < count; i++) {
				$wrap.append('<span class="image-indicator-dot"></span>');
			}
